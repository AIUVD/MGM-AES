...
static void SubBytes1(state_t* state, uint8_t round)
{
	...
	for(uint8_t i = 0; i < 16; i++)
	{
	    ...
	    base = ((uint16_t)((r + i) & 0x0F)) << 8;

	    (*state)[row][col] =
		pgm_read_byte(&sbox1[(*state)[row][col] + base]);
	}
}

static void SubBytes_lr(state_t* state)
{
    uint16_t base;
    ...
    for(uint8_t i = 0; i < 16; i++)
    {
        ...
        base = ((uint16_t)((i + 9) & 0x0F)) << 8;

        (*state)[row][col] =
            pgm_read_byte(&sbox_lr[(*state)[row][col] + base]);
    }
}

static void SubBytes2(state_t* state)
{
    uint16_t base;
    ...
    for(uint8_t i = 0; i < 16; i++)
    {
        ...
        (*state)[row][col] =
            pgm_read_byte(&sbox2[(*state)[row][col] + base]);
    }
}
...
static void Cipher(state_t* state, const uint8_t* RoundKey)
{
	...
	//SubBytes(state);
	ShiftRows(state);
	SubBytes2((state_t*)state2);
	//SubBytes1((state_t*)state2,Nr);
	ShiftRows((state_t*)state2);
...
uint32_t timer32_get(void)
{
    uint32_t value;

    value = ((uint32_t)TCC1.CNT << 16);
...
uint8_t get_pt(uint8_t* pt, uint8_t len)
{
    ...
    out[16] = (uint8_t)(cycles);
    out[17] = (uint8_t)(cycles >> 8);
    out[18] = (uint8_t)(cycles >> 16);
    out[19] = (uint8_t)(cycles >> 24);


    simpleserial_put('r',20,out);*/
